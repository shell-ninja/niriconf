...
menu_select_script="$HOME/.config/niri/scripts/menu.sh"
...
# Scale for monitor
current_mode=$(wlr-randr | awk '/current/ {print $1}')
mon_x_res=$(echo "$current_mode" | cut -d'x' -f1)
mon_scale=$(wlr-randr | awk '/Scale/ {print $2}')
mon_scale_int=$(echo "$mon_scale" | sed 's/\.//')
mon_x_res=$(( mon_x_res * 100 / mon_scale_int ))
